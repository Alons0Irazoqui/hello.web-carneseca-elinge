...
(function initCanvas() {
  ...
  function draw(p) {
    ...
    ctx.shadowColor = p.gold ? 'rgba(201,129,47,0.6)' : 'rgba(255,255,255,0.3)';
    ctx.beginPath();
    ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2);
    ctx.fillStyle = p.gold ? `rgba(201,129,47,${p.op})` : `rgba(243,233,211,${p.op*.5})`;
    ctx.fill();
    ctx.restore();
  }
...
/* ============================================================
   2b. CANVAS PUNTOS DE COLORES DE MARCA — Brasas y chispas flotantes
   Colores: dorado, carne, machaca, chiltepín, mezcal
============================================================ */
(function initColorDots() {
  ...
  const COLORS = [
    { r:201, g:129, b:47,  name:'gold'      },  /* dorado */
    { r:181, g:105, b:31,  name:'carne'     },  /* Carne Seca */
    { r:138, g:74,  b:42,  name:'machaca'   },  /* Machaca */
    { r:192, g:57,  b:43,  name:'chiltepin' },  /* Chile Chiltepín */
    { r:184, g:134, b:11,  name:'mezcal'    },  /* Crema de Mezcal */
    { r:242, g:194, b:126, name:'gold2'     },  /* dorado claro */
  ];
...
function splitTitleIntoLines() {
  ...
  const textLines = [
    ...
    'con el sabor auténtico',
  ];
  const emText = 'del rancho zacatecano.';
...
(function initHeroParallax() {
  ...
  function tick() {
    ...
    /* Sello artesanal: parallax más suave y en dirección opuesta para efecto profundidad */
    if (waterDrop) {
      const dx = cx * -8, dy = cy * -6;
      waterDrop.style.transform = `translate(${dx}px, ${dy}px)`;
    }
...
(function initCursorGlow() {
  ...
  g.style.cssText = `position:fixed;pointer-events:none;z-index:0;width:340px;height:340px;border-radius:50%;background:radial-gradient(circle,rgba(201,129,47,0.05) 0%,transparent 70%);transform:translate(-50%,-50%);transition:left 0.12s ease,top 0.12s ease,opacity 0.4s;will-change:left,top;opacity:0;`;
...
function handleFormSubmit(e) {
  ...
  let waText = `Hola, me comunico desde la página web de Carne Seca El Inge.\n\n`;
  ...
  setTimeout(() => {
    const waURL = `https://wa.me/526145540702?text=${encodeURIComponent(waText)}`;
